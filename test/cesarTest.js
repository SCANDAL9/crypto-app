...
function assertEquals(actual, expected, testName) {
  if (actual === expected) {
    ...
    console.error(
      `❌ ${testName} falló. Esperado: "${expected}", pero obtuve: "${actual}"`
    );
  }
}
...
const t1 = "HELLOWORLD";
const c1 = cifrarCesar(t1, 5, { normalize: "uppercase" });
const d1 = descifrarCesar(c1, 5, { normalize: "uppercase" });
assertEquals(d1, t1, "Caesar roundtrip");

// Test 2: shift 26 debe dar identidad
const t2 = "ABCXYZ";
const c2 = cifrarCesar(t2, 26, { normalize: "uppercase" });
